...
public class RTMPConnectionUtils {
	public static void returnResultForInvoke(
			...
			RTMPInvoke invoke) {
		RTMPInvoke reply = getReplyForInvoke(result, invoke);
		connection.writePacket(reply);
	}
	
	private static RTMPInvoke getReplyForInvoke(Object result, RTMPInvoke invoke) {
		Object resultToReply = result;
		...
		if (result instanceof RTMPStatus) {
			...
			resultToReply = statusMap;
			if (RTMPStatus.LEVEL_ERROR.equals(status.getLevel())) {
				isError = true;
			}
		} else if (result instanceof Throwable) {
			isError = true;
		}
		RTMPInvoke reply = new RTMPInvoke();
		reply.setStreamId(invoke.getStreamId());
		reply.setChannel(invoke.getChannel());
		if (isError) {
			reply.setAction("_error");
		} else {
			reply.setAction("_result");
		}
		ArrayList<Object> args = new ArrayList<Object>();
		args.add(resultToReply);
		reply.setArguments(args);
		reply.setInvokeId(invoke.getInvokeId());
		reply.setReplyTo(invoke);
		return reply;
	}
