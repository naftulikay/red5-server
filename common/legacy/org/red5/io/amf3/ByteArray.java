...
public class ByteArray implements IDataInput, IDataOutput {
	 ...
	 * @param length
	 */
	protected ByteArray(BufferEx buffer, int length) {
		data = BufferEx.allocate(length);
	...
	public String toString() {
		...
		try {
			data.position(0);
			return data.buf().asCharBuffer().toString();
		} finally {
			data.position(old);
		}
	}
	...
	public void compress() {
		BufferEx tmp = BufferEx.allocate(2);
