...
/**
 * @author Murali Kosaraju Tester class for testing the ThreadPool
 */
public class PoolTester {
	 ...
	 * the results. Please note that we need to set the pool so that the thread
	 * goes back to the pool, upon completion of the task.
	 */
	public void runAsyncTask() {
		try {
			log.debug("**** Running runAsyncTask().. ");
			WorkerThread rt1 = (WorkerThread) pool.borrowObject();
			// set the pool now so that the WorkerThread knows how to return
			// itself
			// to the pool.
			rt1.setPool(pool);
