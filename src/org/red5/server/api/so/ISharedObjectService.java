...
public interface ISharedObjectService {
	...
	/**
	 * <p>
	 * Deletes persistent shared objects specified by name and clears all properties from
	 * active shared objects (persistent and nonpersistent). The name parameter specifies
	 * the name of a shared object, which can include a slash (/) as a delimiter between 
	 * directories in the path. The last element in the path can contain wildcard patterns
	 * (for example, a question mark [?] and an asterisk [*]) or a shared object name. The
	 * clearSharedObjects() method traverses the shared object hierarchy along the specified
	 * path and clears all the shared objects. Specifying a slash (/) clears all the shared
   ...
   * / clears all local and persistent shared objects associated with the instance.
	 * <br />
   * /foo/bar clears the shared object /foo/bar; if bar is a directory name, no shared
   *  objects are deleted.
	 * <br />
   * /foo/bar/* clears all shared objects stored under the instance directory /foo/bar.
   *  The bar directory is also deleted if no persistent shared objects are in use within
   *  this namespace.
	 * <br />
   * /foo/bar/XX?? clears all shared objects that begin with XX, followed by any two
   *  characters. If a directory name matches this specification, all the shared objects
   *  within this directory are cleared.
   * </p>
   * <p>
   * If you call the clearSharedObjects() method and the specified path matches a shared
   * object that is currently active, all its properties are deleted, and a "clear" event
   * is sent to all subscribers of the shared object. If it is a persistent shared object,
   * the persistent store is also cleared.
   * </p>
   * <br />
	 * @param scope	the scope to check for the shared object
	 * @param name the name of the shared object
	 * @return true if the shared object at the specified path was deleted; otherwise, false.
	 * If using wildcard characters to delete multiple files, the method returns true only if
	 * all the shared objects matching the wildcard pattern were successfully deleted;
