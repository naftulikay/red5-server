...
public interface ITokenBucket {
	/**
	 * Acquire tokens amount of <tt>tokenCount</tt> waiting <tt>wait</tt>
	 * milliseconds if token not available.
	 * 
	 * @param tokenCount
	 *            The count of tokens to acquire.
	 * @param wait
	 *            Milliseconds to wait. <tt>0</tt> means no wait and any value
	 *            below zero means wait forever.
	 * @return <tt>true</tt> if successfully acquired or <tt>false</tt> if
	 *         not acquired.
	 */
	...
	/**
	 * Nonblockingly acquire token. The upper limit is specified. If not enough
	 * tokens are left in bucket, all remaining will be returned.
	 * 
	...
	/**
	 * Reset this token bucket. All pending threads are woken up with
	 * <tt>false</tt> returned for acquiring token and callback is removed w/o
	 * calling back.
	 */
	void reset();
